...
class EnvConfig:
    ...
    camera_eye: list[float] = field(
        default_factory=lambda: [1.9542544578949033, 0.8892139220503694, 1.1613520627265954]
    )
    camera_target: list[float] = field(
        default_factory=lambda: [1.0874663592651088, 0.44882757271693136, 0.9273899816226002]
    )
    light_intensity: float = 1200.0
    wrist_camera_warmup_steps: int = 5
    wrist_camera_resolution: tuple[int, int] = (640, 480)
    wrist_camera_translation: np.ndarray = field(default_factory=lambda: np.array([0.0, 0.0, 0.1]))
    wrist_camera_name: str = "wrist_camera"
    wrist_camera_prim_name: str = "WristCamera"
    wrist_link_name: str = "link7"
    wrist_rgb_key: str = "wrist_rgb"
    left_camera_resolution: tuple[int, int] = (640, 480)
    right_camera_resolution: tuple[int, int] = (640, 480)
    left_camera_translation: np.ndarray = field(default_factory=lambda: np.array([0.8, 0.4, 0.8]))
    right_camera_translation: np.ndarray = field(default_factory=lambda: np.array([0.8, -0.4, 0.8]))
    ...
    left_rgb_key: str = "left_rgb"
    right_rgb_key: str = "right_rgb"
...
class XArmIsaacEnvironment(_environment.Environment):
    ...
    def __init__(self, cfg: EnvConfig) -> None:
        ...
        self._wrist_camera.set_local_pose(translation=cfg.wrist_camera_translation)
        self._left_camera = Camera(
            prim_path=f"{cfg.table_prim_path}/{cfg.left_camera_prim_name}",
            name=cfg.left_camera_name,
            resolution=cfg.left_camera_resolution,
            frequency=1.0 / cfg.rendering_dt,
        )
        self._left_camera.set_local_pose(translation=cfg.left_camera_translation)
        self._right_camera = Camera(
            prim_path=f"{cfg.table_prim_path}/{cfg.right_camera_prim_name}",
            name=cfg.right_camera_name,
            resolution=cfg.right_camera_resolution,
            frequency=1.0 / cfg.rendering_dt,
        )
        self._right_camera.set_local_pose(translation=cfg.right_camera_translation)
    ...
    def apply_action(self, action: dict) -> None:
        if self._xarm is not None and "actions" in action:
            targets = np.asarray(action["actions"], dtype=np.float32)
            if targets.size:
    ...
    def _collect_observation(self) -> dict:
        ...
        if self._wrist_camera is not None:
            if hasattr(self._wrist_camera, "get_rgb"):
                ...
                if rgb is not None:
                    images[self.cfg.wrist_rgb_key] = np.asarray(rgb)
            else:
                rgba = self._wrist_camera.get_rgba()
                if rgba is not None:
                    images[self.cfg.wrist_rgb_key] = np.asarray(rgba)[..., :3]
        if self._left_camera is not None:
            if hasattr(self._left_camera, "get_rgb"):
                rgb = self._left_camera.get_rgb()
                if rgb is not None:
                    images[self.cfg.left_rgb_key] = np.asarray(rgb)
            else:
                rgba = self._left_camera.get_rgba()
                if rgba is not None:
                    images[self.cfg.left_rgb_key] = np.asarray(rgba)[..., :3]
        if self._right_camera is not None:
            if hasattr(self._right_camera, "get_rgb"):
                rgb = self._right_camera.get_rgb()
                if rgb is not None:
                    images[self.cfg.right_rgb_key] = np.asarray(rgb)
            else:
                rgba = self._right_camera.get_rgba()
                if rgba is not None:
                    images[self.cfg.right_rgb_key] = np.asarray(rgba)[..., :3]
        return {
            "state": np.asarray(state if state is not None else [], dtype=np.float32),
            "images": images,
        }
